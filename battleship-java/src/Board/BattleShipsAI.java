...
public class BattleShipsAI implements Serializable {
    ...
    public void putShips(AbstractShip ships[]) {
        int x, y;
        AbstractShip.Orien o;
        Random rnd = new Random();				//ici on est en train d'utiliser le method random
        AbstractShip.Orien[] orientations = AbstractShip.Orien.values();

        for (AbstractShip s : ships) {
            do {
                // use Random to pick a random x, y & orientation
            	x = rnd.nextInt(size);					//random x					
            	y = rnd.nextInt(size);					//random y
            	o = orientations[rnd.nextInt(4)];		//random orientation
            	s.setorientation(o);
    ...
    private boolean canPutShip(AbstractShip ship, int x, int y) {
        AbstractShip.Orien o = ship.getorientation();
        int dx = 0, dy = 0;
        if (o == AbstractShip.Orien.EAST) {
            if (x + ship.gettaille() >= this.size) {
                return false;
            }
            dx = 1;
        } else if (o == AbstractShip.Orien.SOUTH) {
            if (y + ship.gettaille() >= this.size) {
                return false;
            }
            dy = 1;
        } else if (o == AbstractShip.Orien.NORTH) {
            if (y + 1 - ship.gettaille() < 0) {
                return false;
            }
            dy = -1;
        } else if (o == AbstractShip.Orien.WEST) {
    ...
    public int[] pickRandomCoord() { //j'ai changé de private au public pour la pouvoir utiliser dasn TestGame(main)
